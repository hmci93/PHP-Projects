...
//$webUrl = "http://localhost/rhino-bamboo/admin/"; 
function getRoutesFromCityName($id,$c){
global $conn;	
global $webUrl;
			if($c==1)
			$sqlV = "Select distinct(RouteFrom) from  jh_routes";
			elseif($c==2)
			 $sqlV = "Select * from  jh_routes where RouteFrom='$id'";
			$resV = mysqli_query($conn,$sqlV);
			$r = 0;
					while($row = mysqli_fetch_array($resV)){
						$newsArray[$r][0] = $row['RouteID'];
						$newsArray[$r][1] = $row['RouteNo'];
						$newsArray[$r][2] = $row['RouteFrom'];
						$newsArray[$r][3] = $row['RouteTo'];
						$newsArray[$r][4] = $row['ReportingTime'];
						$newsArray[$r][5] = $row['DepartTime'];
						$newsArray[$r][6] = $row['JourneyDuration'];
						$newsArray[$r][7] = $row['RouteStatus'];
						$newsArray[$r][8] = $row['CreatedOn'];
						$newsArray[$r][9] = $row['CreatedBy'];
						$newsArray[$r][10] = $row['EditedOn'];
						$newsArray[$r][11] = $row['EditedBy'];
						$r++;
					}		
			//mysqli_close($conn);	
			return $newsArray;	
...
global $conn;
			$sqlV = "Select * from  jh_cities where CityID='$id'";
			$resV = mysqli_query($conn,$sqlV);
			$r = 0;
					while($row = mysqli_fetch_array($resV)){
						$newsArray[$r][0] = $row['CityID'];
						$newsArray[$r][1] = $row['CityName'];
						$newsArray[$r][2] = $row['CitySName'];
						$newsArray[$r][3] = $row['CityStatus'];
						$newsArray[$r][4] = $row['CreatedOn'];
						$newsArray[$r][5] = $row['CreatedBy'];
						$newsArray[$r][6] = $row['EditedOn'];
						$newsArray[$r][7] = $row['EditedBy'];
						$r++;
					}		
			//mysqli_close($conn);
			return $newsArray;	
...
if(time()-$stime < 60*30)
 ...
 alert("Your login session is timed out. please login again.");
	window.location.href= "<?php echo $webUrl;?>admin/logout.php";
 </script>
 <?php   
  //logout();
 exit;
