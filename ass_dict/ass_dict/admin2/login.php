...
class clogin extends crole {
	...
	function __construct() {
		...
		// Table object (role)
		if (!isset($GLOBALS["role"]) || get_class($GLOBALS["role"]) == "crole") {
			$GLOBALS["role"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["role"];
		}
		if (!isset($GLOBALS["role"])) $GLOBALS["role"] = &$this;
	...
	function Page_Main() {
		...
		if (IsLoggingIn()) { // After changing password
			...
			if (isset($_POST["username"])) {
				...
				$sPassword = MD5(ew_RemoveXSS(@$_POST["password"]));
