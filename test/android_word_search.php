...
$sql="SELECT word, sound, explanation, english, image from word where word='$search_word' or transliterate='$search_word'";
$result=mysqli_query($conn,$sql);
$soundex=assoundex($search_word);
//echo $soundex;
$sql1="SELECT word FROM word WHERE as_soundex LIKE '$soundex%' order by LOCATE('$soundex', word)";
...
if(mysqli_num_rows($result)==0 && !empty($soundex)){
    if(mysqli_num_rows($result1)>0){
        while($row1 = mysqli_fetch_assoc($result1)){
        $details1 = json_encode($row1);
            echo $details1;
        }
    }
}
