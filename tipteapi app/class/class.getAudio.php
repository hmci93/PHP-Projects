<?php
 ...
 class audio{
	...
	//GETTER METHOD CREATION....!
	 ...
	 function getassetAudio(){ return $this->assetAudio;}
	...
	//SETTER METHOD CREATION..!
	 ...
	 function setassetAudio($assetAudio_v){ $this->assetAudio=$assetAudio_v;}
	...
	function insert()
	{ $sql="INSERT INTO `audio`(category,userId,dateTime,place,assetAudio,reserved1,reserved2,reserved3,reserved4,reserved5) VALUES ('$this->category','$this->userId','$this->dateTime','$this->place','$this->assetAudio','$this->reserved1','$this->reserved2','$this->reserved3','$this->reserved4','$this->reserved5')";
	...
	function getDataByUser($u_id)
	{
        ...
        $sql="SELECT `id` AS `a_id`,`category`,`userId`,`dateTime`,`place`,`assetAudio`,`reserved1` AS `no_of_likes` from `audio` WHERE `userId` = '".$u_id."' ORDER BY `id` DESC";
        $result =$this->database->query($sql);
	...
	function getSingleData($a_id) {
		
		$arr = array();

		$sql = "SELECT `id` AS `a_id`,`category`,`userId`,`dateTime`,`place`,`assetAudio`,`reserved1` AS `no_of_likes` from `audio` WHERE `id` = ".$a_id;
		$result = $this->database->query($sql);
		$result = $this->database->result;
		if ($this->database->rows==0) {
            $ar = array("message" => "Sorry.....Record Doesn't Exist");
	...
	function getDataByCategory($cat)
	{
        ...
        $sql="SELECT `id` AS `a_id`,`category`,`userId`,`dateTime`,`place`,`assetAudio`,`reserved1` AS `no_of_likes` from `audio` WHERE `category` = '".$cat."' ORDER BY `id` DESC";
        $result =$this->database->query($sql);
        $result =$this->database->result;
        if ($this->database->rows==0) {
            $ar = array("message" => "Sorry.....Record Doesn't Exist");
	...
	function getSpecificData($cat, $u_id)
	{
        ...
        $sql="SELECT `id` AS `a_id`,`category`,`userId`,`dateTime`,`place`,`assetAudio`,`reserved1` AS `no_of_likes` from `audio` WHERE `userId` = '".$u_id."' AND `category` = '".$cat."' ORDER BY `id` DESC";
        $result =$this->database->query($sql);
        $result =$this->database->result;
        if ($this->database->rows==0) {
            $ar = array("message" => "Sorry.....Record Doesn't Exist");
    ...
    function getAllData() {
		...
		$sql = "SELECT `id` AS `a_id`, `category`, `userId`, `dateTime`, `place`, `assetAudio`, `reserved1` AS `no_of_likes` FROM `audio` ORDER BY `id` DESC";
		$result = $this->database->query($sql);
		$result = $this->database->result;
	...
	function deleteAudio($a_id) {
		$sql = "DELETE FROM `audio` WHERE `id` = ".$a_id;
		$result = $this->database->query($sql);
		$result = $this->database->result;
	...
	function updateData() {
		$sql = "UPDATE `audio` SET `category` = '".$this->category."', `userId` = '".$this->userId."', `place` = '".$this->place."' WHERE `id` = $this->id";
		$result=$this->database->query($sql);
		if ($this->database->affected_rows > 0) {
	...
	function updateAudioName() {
		$sql = "UPDATE `audio` SET `assetAudio` = '".$this->assetAudio."' WHERE `id` = $this->id";
		$result=$this->database->query($sql);
		if ($this->database->affected_rows > 0) {
			$ar = array('message' => 'Updated Successfully');
	        echo json_encode($ar);
