...
class followers {
    ...
    function getUserList($list) {
        ...
        foreach ($l1_obj as $key) {
            $sql1 = "SELECT `id` AS `up_id`, `username`, `user_following`, `user_followed_by`, `prof_img_path`, `reserved1` AS `displayName`, `reserved2` AS `userStatus` FROM `user_profile` WHERE `id` = ".$key['follower_user_id'];
    ...
    function getFollowerForMap($userId) {
        $folr_arr = array();
        $sql = "SELECT `follower_user_id` FROM `followers` WHERE `user_id` = $userId";
        $result = $this->database->query($sql);
        $result = $this->database->result;
